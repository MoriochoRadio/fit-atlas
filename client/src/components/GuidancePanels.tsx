import { ArrowRight, CheckCircle2, RotateCcw, ShieldCheck, Star } from "lucide-react";
...
import { readLocalWellnessPreferences, saveLocalWellnessPreferences, type RecoveryReflection, type RecoveryRoutineRecord } from "@/lib/wellnessPreferences";
...
function RecoveryHistoryTimeline({ history, onResume }: { history: RecoveryRoutineRecord[]; onResume: (duration: SeatedRecoveryDuration) => void }) {
  if (history.length === 0) return null;
  return <section className="recovery-history" aria-label="최근 회복 수행 기록"><div className="recovery-history-heading"><div><p className="small-label">RECENT RECOVERY · LOCAL ONLY</p><h4>최근 회복 기록</h4><p>이 브라우저에 저장된 최근 {history.length}회입니다.</p></div><RotateCcw size={18} /></div><ol>{history.map((record, index) => <li key={`${record.completedAt ?? record.completedOn}-${record.duration}-${index}`}><div><b>{record.duration}분 자리 회복</b><span>{record.completedOn} · {record.reflection ? reflectionLabels[record.reflection] : "체감 미입력"}</span></div><button type="button" onClick={() => onResume(record.duration)}>다시 열기 <ArrowRight size={14} /></button></li>)}</ol></section>;
}
...
export function SeatedRecoveryPanel({ duration, onDuration, recommendation, recoveryContext, onExplore, onBuildSession }: { duration: SeatedRecoveryDuration; onDuration: (duration: SeatedRecoveryDuration) => void; recommendation: CheckinRecommendation; recoveryContext: RecoveryContext; onExplore: (exerciseId: string) => void; onBuildSession: () => void }) {
  ...
  const completeRoutine = () => {
    ...
    const record: RecoveryRoutineRecord = { duration, completedOn: completedAt.slice(0, 10), completedAt, reflection: null };
