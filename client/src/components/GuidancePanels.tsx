...
function RecoveryHistoryTimeline({ history, onResume, onDelete, onClear }: { history: RecoveryRoutineRecord[]; onResume: (duration: SeatedRecoveryDuration) => void; onDelete: (record: RecoveryRoutineRecord) => void; onClear: () => void }) {
  ...
  const recentReflections = recentFour.map((record) => record.reflection).filter((reflection): reflection is RecoveryReflection => Boolean(reflection));
  const nextChoice = recentReflections.length === 0 ? "체감을 한 번 남기면 다음 기록부터 4회 흐름에 맞춘 회복 시간을 비교합니다." : recentReflections[0] === "pause" ? "최근 쉬어가기 신호가 있으니, 다음 회복은 5분 루틴으로 범위와 반응만 먼저 확인하세요." : recentReflections.slice(0, 2).every((reflection) => reflection === "lighter") ? "최근 가벼워짐 흐름입니다. 같은 시간대를 유지하거나, 반응이 안정적일 때만 10분 루틴으로 넓히세요." : "체감이 섞여 있습니다. 다음 회복에서도 같은 시간대를 유지하고 메모로 환경 변화를 함께 남겨 보세요.";
