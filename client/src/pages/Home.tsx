...
export default function Home() {
  ...
  return (
    <div className={`site-shell scene-${activeScene}`}>
      ...
      {storageUnavailable && <div className="storage-warning" role="status"><ShieldCheck size={16} /><p><strong>이 브라우저에서 자동 저장이 제한되었습니다.</strong> 현재 화면의 변경 사항은 유지되지만, 탭을 닫으면 사라질 수 있습니다. 브라우저 저장 공간 또는 개인정보 보호 설정을 확인한 뒤 다시 시도해 주세요.</p></div>}
...
function TextExerciseGuide({ guide, exerciseName }: { guide: ExerciseTextGuide; exerciseName: string }) {
  return <section className="text-exercise-guide" aria-label={`${exerciseName} 사진 없는 자세 안내`}><div className="text-guide-head"><div><p className="small-label">TEXT MOVEMENT MAP</p><h4>읽으며 따라 하는 자세 지도</h4></div><span>사진 없이도<br />① → ② → ③</span></div><ol className="text-guide-sequence">{guide.sequence.map((step, index) => <li key={step}><span>0{index + 1}</span><div><b>{["시작 자세", "움직임", "마무리 확인"][index]}</b><p>{step}</p></div></li>)}</ol><div className="text-guide-facts"><article><span>◎ 주로 쓰는 근육</span><p>{guide.primaryMuscles.join(" · ")}</p></article><article><span>＋ 함께 쓰는 근육</span><p>{guide.supportingMuscles.join(" · ")}</p></article><article><span>↔ 호흡</span><p>{guide.breathing.replace("↔ ", "")}</p></article><article><span>↓ 어렵다면</span><p>{guide.adjustment.replace("↓ ", "")}</p></article></div><p className="text-guide-stop">{guide.stop}</p></section>;
}
