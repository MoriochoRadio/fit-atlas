...
const HeroGymMachine3D = ({ goal, environment, completion, nodes, onOpenNode }: { goal: SessionGoal; environment: SessionEnvironment; completion: number; nodes: string[]; onOpenNode: (index: number) => void }) => <Suspense fallback={<div className="hero-machine-loading" role="status">오늘의 장비를 준비하는 중</div>}><LazyHeroGymMachine3D goal={goal} environment={environment} completion={completion} nodes={nodes} onOpenNode={onOpenNode} /></Suspense>;
...
export default function Home() {
  ...
  return (
    <AsciiInteractionContext.Provider value={{ showAxis: axisVisible, pendingExerciseName, clearPendingExercise: () => setPendingExerciseName(null), onOpenRom: (exerciseName, presentation) => setRomRecommendationTarget({ exerciseName, presentation }), onExploreAlternative: (exerciseName) => { setRomRecommendationTarget(null); setKeyword(exerciseName); setCategory("전체"); setFocus("전체"); setRegionFilter("전체"); setDifficulty("전체"); setEquipment("전체"); setRomFilter("전체"); setPendingExerciseName(exerciseName); setActiveScene("explore"); window.setTimeout(() => document.getElementById("explore")?.scrollIntoView({ behavior: "smooth" }), 0); }, onAddToTodayRoutine: (exerciseName) => { setWeeklyPlan((current) => addRomAlternativeToWeeklyPlan(current, exerciseName)); toast.success(`${exerciseName}을 오늘의 운동 루틴에 추가했습니다.`); } }}><div className={`site-shell scene-${activeScene} atlas-theme-${atlasTheme} atlas-motion-${sessionGoal} atlas-environment-${sessionEnvironment} atlas-speed-${atlasInteraction.motionSpeed} atlas-performance-${atlasPerformance}${atlasTransition ? ` atlas-transition-${atlasTransition}` : ""}`}>
      ...
      <main id="top">
        <nav className="mobile-quick-nav" aria-label="모바일 빠른 이동"><a href="#explore"><Search size={17} /><span>탐색</span></a><a href="#session"><Timer size={17} /><span>오늘 세션</span></a><button onClick={() => setLogOpen(true)}><Plus size={18} /><span>기록</span></button><a href="#recovery"><HeartPulse size={17} /><span>회복</span></a></nav>
        <section className="hero">
          ...
          <div className="hero-atlas" aria-label={`오늘의 ${sessionDuration}분 ${sessionGoal === "strength" ? "기초 근력" : sessionGoal === "endurance" ? "심폐 리듬" : "전신 균형"} 세션 요약`}>
            ...
            <HeroGymMachine3D goal={sessionGoal} environment={sessionEnvironment} completion={weeklyCompletionPercent} nodes={atlasBlocks.slice(0, 3).map((block) => block.label)} onOpenNode={openAtlasNode} />
