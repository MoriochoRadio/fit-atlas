      ...
      <main id="top">
        ...
        <section className="hero">
          ...
          <div className="hero-atlas" aria-label={`오늘의 ${sessionDuration}분 ${sessionGoal === "strength" ? "기초 근력" : sessionGoal === "endurance" ? "심폐 리듬" : "전신 균형"} 세션 요약`}>
            ...
            <div className="motion-map"><span className="motion-scan" aria-hidden="true" /><span className="motion-ripple ripple-a" aria-hidden="true" /><span className="motion-ripple ripple-b" aria-hidden="true" /><span className="atlas-particles" aria-hidden="true">{Array.from({ length: 12 }, (_, index) => <i key={index} />)}</span>{atlasBlocks.slice(0, 3).map((block, index) => <button key={`${block.label}-${index}`} type="button" className={`route-node node-${["a", "b", "c"][index]}`} onClick={() => openAtlasNode(index)} aria-haspopup="dialog" aria-label={`${block.label} 블록 상세 및 편집`}>{String(index + 1).padStart(2, "0")}</button>)}</div>
