...
const categories = ["전체", "러닝", "유산소", "헬스기구", "프리웨이트", "맨몸운동", "모빌리티"] as const;
...
export default function Home() {
  ...
  const addLog = () => {
    ...
    const entry = { id: crypto.randomUUID(), date: form.date, exercise: form.exercise, sets: Number(form.sets), reps: Number(form.reps), load: Number(form.load), minutes: Number(form.minutes), intensity: Number(form.intensity) };
    ...
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section id="explore" className="explore-section section-pad">
          <SectionTitle eyebrow="EXERCISE LIBRARY" title="움직임을 지식으로 익히세요." description={`개인용 정적 큐레이션: 6개 카테고리 · ${exercises.length}개 운동. 카테고리와 목적, 장비로 탐색하고 올바른 자세·효과·안전 단서를 확인하세요.`} action={<span className="library-count">{filteredExercises.length} EXERCISES</span>} />
