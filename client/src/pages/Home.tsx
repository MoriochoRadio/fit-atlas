...
export default function Home() {
  ...
  const [form, setForm] = useState(() => ({ date: new Date().toISOString().slice(0, 10), exercise: "바벨 백 스쿼트", sets: "3", reps: "8", load: "40", minutes: "35", distanceKm: "", intensity: "6" }));
  ...
  const addLog = () => {
    const today = new Date().toISOString().slice(0, 10);
    const distanceKm = form.distanceKm.trim() ? Number(form.distanceKm) : undefined;
    const entry = { id: crypto.randomUUID(), date: form.date, exercise: form.exercise, sets: Number(form.sets), reps: Number(form.reps), load: Number(form.load), minutes: Number(form.minutes), intensity: Number(form.intensity), ...(distanceKm ? { distanceKm } : {}) };
    if (!/^\d{4}-\d{2}-\d{2}$/.test(entry.date) || entry.date > today) {
      toast.error("운동 날짜는 오늘 또는 과거 날짜로 입력해 주세요.");
      return;
    }
    if (!Number.isInteger(entry.sets) || entry.sets < 1 || entry.sets > 30 || !Number.isInteger(entry.reps) || entry.reps < 1 || entry.reps > 500 || !Number.isFinite(entry.load) || entry.load < 0 || entry.load > 1000 || !Number.isInteger(entry.minutes) || entry.minutes < 1 || entry.minutes > 1440 || !Number.isInteger(entry.intensity) || entry.intensity < 1 || entry.intensity > 10 || (distanceKm !== undefined && (!Number.isFinite(distanceKm) || distanceKm <= 0 || distanceKm > 500))) {
  ...
  const startPlanSessionLog = (session: WeeklyPlan["sessions"][number]) => {
    ...
    setForm((current) => ({ ...current, date: new Date().toISOString().slice(0, 10), exercise: exerciseByPlan[session.goal][session.environment], sets: session.goal === "endurance" ? "1" : "2", reps: session.goal === "endurance" ? "1" : "8", load: "0", minutes: String(session.duration), distanceKm: "", intensity: baseIntensity }));
