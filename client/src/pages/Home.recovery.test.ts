...
describe("Home recovery alternative flow", () => {
  ...
  it("updates the search and region filter, then opens the independent library scene when a pathway alternative is clicked", async () => {
    ...
    const ankleAlternative = screen
      .getAllByRole("button")
      .find(button => button.textContent?.includes("발목 니투월 락"));
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "부위·목적·난이도 상세 조건" })
    );
    expect(
      (screen.getByLabelText("부위 필터") as HTMLSelectElement).value
    ).toBe("하체");
    await waitFor(() =>
      expect(
        document
          .querySelector(".site-shell")
          ?.classList.contains("scene-explore")
      ).toBe(true)
    );
  });
  ...
  it("renders distance units and the four-week rhythm card in the local record experience", () => {
    ...
    fireEvent.click(
      within(topbar as HTMLElement).getByRole("button", { name: /운동 기록/ })
    );
  ...
  it("synchronizes the 3D muscle model and text list, then opens a related exercise detail", async () => {
    ...
    const model = await waitFor(() =>
      screen.getByLabelText("클릭·드래그 가능한 3D 근육 인체 모델")
    );
    ...
    expect(
      within(model)
        .getByRole("button", { name: "후면" })
        .getAttribute("aria-pressed")
    ).toBe("true");
    expect(
      screen.getByRole("button", { name: "등" }).getAttribute("aria-pressed")
    ).toBe("true");
    expect(screen.getByRole("heading", { name: "등" })).toBeTruthy();

    const relatedExercise = document.querySelector(
      ".anatomy-exercise-list button"
    ...
    await waitFor(() =>
      expect(
        (screen.getByLabelText("운동 검색") as HTMLInputElement).value
      ).toBe(exerciseName)
    );
    expect(
      document.querySelector(".site-shell")?.classList.contains("scene-explore")
    ).toBe(true);
  });
  ...
  it("rotates the muscle model, filters with multiple regions, and exposes exercise muscle roles", async () => {
    ...
    const multiSelect = screen.getByRole("group", {
      name: "근육 부위 다중 선택",
    });
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: "자세·근거 보기" })[0]!
    );
    await waitFor(() =>
      expect(screen.getByLabelText(/근육 역할/)).toBeTruthy()
    );
    expect(
      document.querySelectorAll(".muscle-zone.is-primary").length
    ).toBeGreaterThan(0);
  });
  ...
  it("장비를 고르고 강도를 조절하면 세션 안내에 반영된다", async () => {
    ...
    const picker = await waitFor(() =>
      screen.getByRole("group", { name: "장비 선택" })
    );
    fireEvent.click(within(picker).getByRole("button", { name: "덤벨" }));
    expect(
      within(picker)
        .getByRole("button", { name: "덤벨" })
        .getAttribute("aria-pressed")
    ).toBe("true");
  ...
  it("장비를 바꾸고 강도를 올리면 현재 세션 기준에 반영된다", async () => {
    ...
    fireEvent.click(within(picker).getByRole("button", { name: "덤벨" }));
    fireEvent.change(screen.getByLabelText("세션 강도") as HTMLInputElement, {
      target: { value: "78" },
    });
    await waitFor(() =>
      expect(screen.getAllByText(/78% · 집중 RPE 8/).length).toBeGreaterThan(0)
    );
  });
  ...
  it("renders the action-first start panel and opens the independent current session scene", async () => {
    render(createElement(Home));
    expect(
      screen.getByRole("heading", { name: /오늘은\s*무엇을 움직일까요\?/ })
    ).toBeTruthy();
    expect(
      screen.getByLabelText("오늘의 30분 전신 균형 세션 요약")
    ).toBeTruthy();
    ...
    fireEvent.click(
      within(startDock).getByRole("button", { name: /오늘 세션/ })
    );
    ...
    await waitFor(() =>
      expect(
        document
          .querySelector(".site-shell")
          ?.classList.contains("scene-session")
      ).toBe(true)
    );
    expect(
      screen.getByRole("heading", { name: "30분 전신 균형 세션 · 집·매트" })
    ).toBeTruthy();
  });
  ...
  it("히어로에는 상태 요약과 세션 카드만 남기고 외형 설정은 두지 않는다", async () => {
    ...
    expect(
      within(workspace as HTMLElement).getByText("BALANCE ROUTE")
    ).toBeTruthy();
    // 테마·모션 속도는 장면 설정으로 옮겼다.
    expect(
      document.querySelector(".hero-workspace .atlas-theme-control")
    ).toBeNull();
  ...
  it("고른 장비에 맞는 목표로 세션을 시작한다", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "이 장비로 세션 설계" })
    );
    ...
    expect(
      screen.getByRole("heading", { name: "30분 심폐 리듬 세션 · 집·매트" })
    ).toBeTruthy();
  });
  ...
  it("switches the main navigation through independent explore, body, progress, and wellness scenes", async () => {
    ...
    fireEvent.click(
      within(primaryNav).getByRole("link", { name: "운동 탐색" })
    );
    await waitFor(() =>
      expect(
        document
    ...
    expect(
      screen.getByRole("heading", { name: "움직임을 지식으로 익히세요." })
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(
        document
          .querySelector(".site-shell")
          ?.classList.contains("scene-anatomy")
      ).toBe(true)
    );
    expect(
      screen.getByRole("heading", {
        name: "부위를 누르면, 필요한 움직임이 보입니다.",
      })
    ).toBeTruthy();

    fireEvent.click(
      within(primaryNav).getByRole("link", { name: "기록 분석" })
    );
    await waitFor(() =>
      expect(
        document
          .querySelector(".site-shell")
          ?.classList.contains("scene-progress")
      ).toBe(true)
    );
    expect(
      screen.getByRole("heading", { name: "기록은 감이 아닌 방향을 만듭니다." })
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(
        document
          .querySelector(".site-shell")
          ?.classList.contains("scene-wellness")
      ).toBe(true)
    );
    expect(
      screen.getByRole("heading", {
        name: "오래 앉은 뒤, 다음 작업을 위한 짧은 전환.",
      })
    ).toBeTruthy();
  });
  ...
  it("toggles scene transition sound from the dedicated settings dialog", async () => {
    ...
    const topbar = document.querySelector(".topbar");
    fireEvent.click(
      within(topbar as HTMLElement)
        .getAllByRole("button", { name: "장면 설정" })
        .find(button => button.classList.contains("desktop-only"))!
    );
    const dialog = await waitFor(() =>
      screen.getByRole("dialog", { name: /장면 전환 설정/ })
    );
    ...
    expect(
      JSON.parse(
        window.localStorage.getItem("fit-atlas-scene-experience") ?? "{}"
      )
    ).toMatchObject({ soundEnabled: false });
  });
  ...
  it("opens scene settings from the mobile navigation entry without leaving its menu behind", async () => {
    ...
    const mobileSettings = within(navigation)
      .getAllByRole("button", { name: "장면 설정" })
      .find(button => button.classList.contains("mobile-only"));
  ...
  it("saves, applies, and deletes a named exercise filter preset", async () => {
    ...
    expect(
      screen
        .getByRole("button", { name: "맨몸운동" })
        .getAttribute("aria-pressed")
    ).toBe("true");
    fireEvent.click(
      screen.getByRole("button", { name: "집 운동 프리셋 삭제" })
    );
    expect(
      screen.queryByRole("button", { name: "집 운동 프리셋 삭제" })
    ).toBeNull();
  });
  ...
  it("최근 시작한 장비 세션을 다시 이어간다", async () => {
    ...
    render(createElement(Home));
    expect(
      screen.getByRole("button", { name: "최근 세션 이어하기" })
    ).toBeTruthy();
  ...
  it("shows weekly completion flow, lets the user change the weekly goal, and gives the next direction", () => {
    ...
    expect(
      within(report).queryByRole("heading", { name: "주간 완료 흐름" })
    ).toBeNull();
    expect(within(report).getByRole("button", { name: "근력" })).toBeTruthy();
    expect(
      within(report).getByText(
        "가장 부담이 적은 한 세션을 선택해 이번 주의 첫 신호를 만드세요."
      )
    ).toBeTruthy();
    ...
    expect(
      within(report).getByRole("heading", { name: "주간 완료 흐름" })
    ).toBeTruthy();
    expect(
      within(report).getByText(
        "완료한 세션 하나의 시간과 강도만 기록해 다음 주의 기준을 남겨 보세요."
      )
    ).toBeTruthy();
    fireEvent.click(within(report).getByRole("button", { name: "근력" }));
    expect(
      within(report)
        .getByRole("button", { name: "근력" })
        .getAttribute("aria-pressed")
    ).toBe("true");
    screen
      .getAllByRole("button", { name: /완료 처리/ })
      .forEach(button => fireEvent.click(button));
    expect(
      within(report).getByText(
        "이번 주 계획을 마쳤습니다. 다음 세션에서는 시간·반복·저항 중 하나만 작게 조절하세요."
      )
    ).toBeTruthy();
  });
  ...
  it("opens session design directly from the weekly summary next action", () => {
    ...
    const report = screen.getByLabelText("주간 아틀라스 상세 리포트");
    fireEvent.click(
      within(report).getByRole("button", { name: "다음 세션 설계" })
    );
    expect(
      screen.getByRole("heading", {
        name: "오늘의 조건으로, 한 세션을 설계하세요.",
      })
    ).toBeTruthy();
  });
  ...
  it("applies a quick exercise start path and makes the chosen condition visible", () => {
    ...
    fireEvent.click(
      within(launcher).getByRole("button", { name: /달리기·유산소/ })
    );

    expect(within(launcher).getByText("러닝 · 심폐")).toBeTruthy();
    expect(
      screen.getByRole("button", { name: "러닝" }).getAttribute("aria-pressed")
    ).toBe("true");
  });
  ...
  it("applies a quick session start and makes the current session conditions visible", () => {
    ...
    fireEvent.click(
      within(launcher).getByRole("button", { name: /30분 헬스장/ })
    );

    expect(
      within(launcher).getByText("30분 · 헬스장 · 기초 근력")
    ).toBeTruthy();
    expect(
      screen.getByRole("heading", { name: "30분 기초 근력 세션 · 헬스장" })
    ).toBeTruthy();
    expect(document.querySelector(".site-shell")?.className).toContain(
      "atlas-motion-strength"
    );
  });

  it("reveals session adjustment guidance only on request and exposes wellness quick navigation", () => {
    render(createElement(Home));
    const guidanceToggle = screen.getByRole("button", {
      name: "피로·통증 조절 기준 보기",
    });
    ...
    const wellnessNavigation = screen.getByRole("navigation", {
      name: "웰니스 화면 빠른 이동",
    });
    ["회복 시작", "생활 습관", "유산소", "무점프", "안전"].forEach(name =>
      expect(
        within(wellnessNavigation).getByRole("button", { name })
      ).toBeTruthy()
    );
  });
  ...
  it("장면 설정에서 테마를 바꾸면 저장되고 다시 열어도 유지된다", async () => {
    ...
    const themeGroup = await waitFor(() =>
      screen.getByRole("group", { name: "테마 선택" })
    );
  ...
  it("세션 화면에서 블록을 편집하고, 장면 설정에서 속도를 바꾼다", async () => {
    ...
    fireEvent.click(
      await waitFor(() =>
        screen.getByRole("button", { name: "준비 블록 상세 및 편집" })
      )
    );
    expect(
      screen.getByRole("heading", { name: "준비 블록 편집" })
    ).toBeTruthy();
    fireEvent.change(screen.getByLabelText("블록 이름"), {
      target: { value: "리듬 준비" },
    });
    fireEvent.change(screen.getByLabelText("움직임 · 한 줄에 하나씩"), {
      target: { value: "편안한 걷기\n캣·카우 · 6회" },
    });
    ...
    fireEvent.click(screen.getAllByRole("button", { name: /장면 설정/ })[0]!);
    const speeds = await waitFor(() =>
      screen.getByRole("group", { name: "모션 속도 선택" })
    );
  ...
  it("increases atlas visual density after the current weekly plan is completed", () => {
    render(createElement(Home));
    expect(document.querySelector(".site-shell")?.className).toContain(
      "atlas-performance-starting"
    );
    screen
      .getAllByRole("button", { name: /완료 처리/ })
      .forEach(button => fireEvent.click(button));
    expect(document.querySelector(".site-shell")?.className).toContain(
      "atlas-performance-surge"
    );
  });
  ...
  it("renders the first catalog page first, then appends the next page on demand", async () => {
    render(createElement(Home));
    expect(
      screen.getByText(/18개 표시 · \d+\/990개 카탈로그를 불러왔습니다\./)
    ).toBeTruthy();

    fireEvent.click(screen.getByRole("button", { name: /운동 100개 더 보기/ }));
    await waitFor(() =>
      expect(
        screen.getByText(/36개 표시 · \d+\/990개 카탈로그를 불러왔습니다\./)
      ).toBeTruthy()
    );
  });
  ...
  it("saves a favorite and a recently viewed exercise from its card", async () => {
    render(createElement(Home));
    fireEvent.change(screen.getByLabelText("운동 검색"), {
      target: { value: "바벨 백 스쿼트" },
    });
    await waitFor(() =>
      expect(
        screen.getByRole("heading", { name: "바벨 백 스쿼트" })
      ).toBeTruthy()
    );
    const card = screen
      .getByRole("heading", { name: "바벨 백 스쿼트" })
      .closest("article");
    expect(card).toBeTruthy();

    fireEvent.click(
      within(card!).getByRole("button", {
        name: "바벨 백 스쿼트 즐겨찾기 추가",
      })
    );
    expect(
      within(card!)
        .getByRole("button", { name: "바벨 백 스쿼트 즐겨찾기 해제" })
        .getAttribute("aria-pressed")
    ).toBe("true");
    fireEvent.click(
      within(card!).getByRole("button", { name: "자세·근거 보기" })
    );

    await waitFor(() =>
      expect(screen.getAllByText("바벨 백 스쿼트").length).toBeGreaterThan(2)
    );
    expect(screen.getByLabelText("바벨 백 스쿼트 근거 적용 범위")).toBeTruthy();
    expect(
      screen.getByText(/전문 훈련 기관의 기술·훈련 원칙 자료/)
    ).toBeTruthy();
    expect(screen.getByText(/통증·불안정이 있으면 중단/)).toBeTruthy();
    expect(
      screen.getByRole("link", { name: /NSCA 바벨 스쿼트 기술 안내/ })
    ).toBeTruthy();
    ...
    fireEvent.click(
      within(recentPanel).getByRole("button", {
        name: "바벨 백 스쿼트 자세·안전 안내 다시 열기",
      })
    );
    await waitFor(() =>
      expect(
    ...
    await waitFor(() =>
      expect(
        screen.getByLabelText("바벨 백 스쿼트 근거 적용 범위")
      ).toBeTruthy()
    );
  });
  ...
  it("renders the expanded visual guide from a representative bodyweight exercise detail card", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("heading", { name: "맨몸 스쿼트" })).toBeTruthy()
    );
    const card = screen
      .getByRole("heading", { name: "맨몸 스쿼트" })
      .closest("article");
    expect(card).toBeTruthy();
    fireEvent.click(
      within(card!).getByRole("button", { name: "자세·근거 보기" })
    ...
    expect(
      within(card!).getAllByText(
        /발 전체를 바닥에|엉덩이·무릎을 함께|통증 없는 범위로/
      ).length
    ).toBeGreaterThanOrEqual(3);
    expect(
      within(card!).getByLabelText("맨몸 스쿼트 사진 없는 자세 안내")
    ).toBeTruthy();
    ...
    expect(
      within(card!).getByLabelText("맨몸 스쿼트 ASCII 동작 도식")
    ).toBeTruthy();
    expect(
      within(card!).getByText(
        /낮은 범위부터 균형을 유지하는 앉기·일어서기 흐름/
      )
    ).toBeTruthy();
    ...
    expect(
      within(card!).getByLabelText("맨몸 스쿼트 근거 적용 범위")
    ).toBeTruthy();
    expect(
      within(card!).getByRole("button", { name: "간단히 보기" })
    ).toBeTruthy();
    fireEvent.click(within(card!).getByRole("button", { name: "ROM · 큼" }));
    expect(screen.getAllByText(/맨몸 스쿼트 · 큰 ROM/).length).toBeGreaterThan(
      0
    );
    expect(screen.getAllByText("대체 운동 방식").length).toBeGreaterThan(0);
    const alternativeLink = screen
      .getAllByRole("button", { name: /리버스 런지/ })
      .find(element => element.className.includes("alternative-exercise-link"));
    expect(alternativeLink).toBeTruthy();
    fireEvent.click(alternativeLink!);
    await waitFor(() =>
      expect(screen.getByRole("heading", { name: "리버스 런지" })).toBeTruthy()
    );
    const alternativeCard = screen
      .getByRole("heading", { name: "리버스 런지" })
      .closest("article");
    expect(
      within(alternativeCard!).getByText("읽으며 따라 하는 자세 지도")
    ).toBeTruthy();
  });
  ...
  it("adds a ROM alternative to today’s routine and reflects half-step check-ins in the weekly dashboard", async () => {
    ...
    fireEvent.click(within(card!).getByRole("button", { name: "ROM · 큼" }));
    const routineAdd = screen
      .getAllByRole("button", { name: /오늘 루틴/ })
      .find(element => element.className.includes("alternative-routine-add"));
    expect(routineAdd).toBeTruthy();
    fireEvent.click(routineAdd!);
    expect(screen.getByText("오늘의 ROM 조절 · 리버스 런지")).toBeTruthy();
    fireEvent.change(screen.getByRole("slider", { name: /통증/ }), {
      target: { value: "2.5" },
    });
    expect(
      screen.getByLabelText("주간 피로 통증 및 추천 ROM 대시보드")
    ).toBeTruthy();
  });
  ...
  it("장면 설정에서 중심축을 끄고, 라이브러리를 ROM 크기로 거른다", async () => {
    ...
    const axis = await waitFor(() =>
      screen.getByRole("button", { name: "표시" })
    );
  ...
  it("reflects today routine completion in the weekly status dashboard and exposes PNG export", () => {
    render(createElement(Home));
    expect(
      screen.getByRole("button", { name: "주간 상태 그래프 PNG로 내보내기" })
    ).toBeTruthy();
    const completionToggle = screen.getAllByRole("button", {
      name: /완료 처리/,
    })[0];
    fireEvent.click(completionToggle);
    expect(screen.getAllByText("루틴 완료 1/3 · 33%").length).toBeGreaterThan(
      0
    );
  });
  ...
  it("celebrates full routine completion and includes period and note fields in the export dashboard", () => {
    ...
    expect(
      screen.getByLabelText("최근 4주 완료율 및 피로도 변화")
    ).toBeTruthy();
    screen
      .getAllByRole("button", { name: /완료 처리/ })
      .forEach(button => fireEvent.click(button));
  ...
  it("prioritizes a conservative ROM path when today’s pain signal is high", () => {
    ...
    expect(
      screen.getAllByText("통증 신호가 크면 ROM 추천을 잠시 멈추세요").length
    ).toBeGreaterThan(0);
    expect(
      screen.getByRole("button", { name: "회복 가이드 보기" })
    ).toBeTruthy();
  });
  ...
  it("renders the expanded machine visual guide with a safe adjustment cue", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("heading", { name: "랫 풀다운" })).toBeTruthy()
    );
    const card = screen
      .getByRole("heading", { name: "랫 풀다운" })
      .closest("article");
    expect(card).toBeTruthy();
    fireEvent.click(
      within(card!).getByRole("button", { name: "자세·근거 보기" })
    ...
    expect(
      within(card!).getAllByText(/반동 없이 장력 제어/).length
    ).toBeGreaterThan(0);
  });
  ...
  it("loads a non-contact combat starter drill with its visual guide and safety scope", async () => {
    render(createElement(Home));
    fireEvent.change(screen.getByLabelText("운동 검색"), {
      target: { value: "복싱 가드 스텝" },
    });

    await waitFor(() =>
      expect(
        screen.getByRole("heading", { name: "복싱 가드 스텝 리셋" })
      ).toBeTruthy()
    );
    const card = screen
      .getByRole("heading", { name: "복싱 가드 스텝 리셋" })
      .closest("article");
    expect(card).toBeTruthy();
    fireEvent.click(
      within(card!).getByRole("button", { name: "자세·근거 보기" })
    ...
    expect(
      within(card!).getAllByText(/대련·스파링·타격은 포함하지 않습니다/).length
    ).toBeGreaterThan(0);
  });
  ...
  it("switches the seated-work recovery routine and bridges to a light home session", async () => {
    ...
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "10분" })
    );
    expect(
      within(recoveryPanel).getByText("10분 자리 회복·재시작")
    ).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "현재 10분 루틴 저장" })
    );
    expect(
      within(recoveryPanel).getByText("이 10분 루틴을 저장했습니다.")
    ).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getByRole("button", {
        name: "이번 10분 루틴 완료 기록",
      })
    );
    expect(
      within(recoveryPanel).getByText(
        "10분 완료 기록을 남겼습니다. 체감을 선택해 주세요."
      )
    ).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "가벼워짐" })
    );
    expect(within(recoveryPanel).getByText(/체감: 가벼워짐/)).toBeTruthy();
    await waitFor(() =>
      expect(
        screen.getByLabelText("주간 회복 추세 신호").textContent
      ).toContain("가벼워짐")
    );
    await waitFor(() =>
      expect(
        screen.getByLabelText("주간 회복 날짜별 추세").textContent
      ).toContain("↑")
    );
    fireEvent.change(within(recoveryPanel).getByLabelText("이번 회복 메모"), {
      target: { value: "책상 높이를 조정하니 목이 편해짐" },
    });
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "메모 저장" })
    );
    ...
    expect(
      within(recoveryPanel).getByRole("heading", { name: "최근 회복 기록" })
    ).toBeTruthy();
    const recoverySummary =
      within(recoveryPanel).getByLabelText("최근 회복 기록 요약");
    expect(
      within(recoveryPanel).getByLabelText("최근 회복 변화 비교")
    ).toBeTruthy();
    expect(
      within(recoveryPanel).getByLabelText("최근 4회 체감 흐름").textContent
    ).toContain("가벼워짐");
    expect(
      within(recoveryPanel).getByLabelText("최근 체감 기반 다음 회복 선택")
        .textContent
    ).toContain("같은 시간대");
    ...
    expect(
      within(recoveryPanel).getAllByText("자리 회복", { exact: false }).length
    ).toBeGreaterThan(1);
    ...
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "가벼워짐 기록 필터" })
    );
    expect(within(recoveryPanel).getByText("표시 1/2회")).toBeTruthy();
    expect(
      within(recoveryPanel).getByText(
        "가벼워짐 체감 · 메모 “목이” · 최근 변화 비교 유지"
      )
    ).toBeTruthy();
    expect(
      within(recoveryPanel).getByText("책상 높이를 조정하니 목이 편해짐")
    ).toBeTruthy();
    fireEvent.change(memoSearch, { target: { value: "손목" } });
    expect(
      within(recoveryPanel).getByText(
        "선택한 조건과 메모에 맞는 회복 기록이 없습니다."
      )
    ).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "전체 기록 보기" })
    );
    expect(within(recoveryPanel).getByText("표시 2/2회")).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getAllByRole("button", {
        name: /회복 기록 다시 열기/,
      })[1]!
    );
    expect(
    ...
    expect(
      (
        within(recoveryPanel).getByLabelText(
          "이번 회복 메모"
        ) as HTMLTextAreaElement
      ).value
    ).toBe("책상 높이를 조정하니 목이 편해짐");
    fireEvent.click(
      within(recoveryPanel).getByRole("button", {
        name: "10분 자리 회복 기록 삭제",
      })
    );
    expect(
      within(recoveryPanel).getByRole("button", {
    ...
    expect(
      within(recoveryPanel).getByText("이 브라우저에 저장된 최근 1회입니다.")
    ).toBeTruthy();
    expect(within(recoverySummary).getByText("1회")).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "전체 초기화" })
    );
    expect(
      within(recoveryPanel).getByText(
        "이 브라우저에 저장된 회복 기록만 지웁니다. 저장한 루틴은 유지됩니다."
      )
    ).toBeTruthy();
    fireEvent.click(
      within(recoveryPanel).getByRole("button", { name: "전체 기록 삭제" })
    );
    expect(
      within(recoveryPanel).queryByRole("heading", { name: "최근 회복 기록" })
    ).toBeNull();
    expect(
      within(recoveryPanel).queryByLabelText("최근 회복 기록 요약")
    ).toBeNull();
    expect(
      within(recoveryPanel).getByText("이 10분 루틴을 저장했습니다.")
    ).toBeTruthy();
    ...
    expect(
      screen.getByRole("heading", { name: "15분 전신 균형 세션 · 집·매트" })
    ).toBeTruthy();
  });
  ...
  it("provides a low-pressure first record prompt and a quick recovery start", () => {
    ...
    expect(
      screen.getByText(/모든 수치를 완벽히 기억할 필요는 없습니다/)
    ).toBeTruthy();
  });
  ...
  it("renders all four conservative cardio interval templates", () => {
    ...
    expect(screen.getAllByText("수영 짧은 길이 인터벌").length).toBeGreaterThan(
      0
    );
    expect(screen.getAllByText(/RPE 5–6/).length).toBeGreaterThan(0);
    expect(
      screen.getAllByText(/숨이 정리된 뒤만 다음 길이/).length
    ).toBeGreaterThan(0);
  ...
  it("renders 10, 15, 20, and 30 minute low-noise no-jump circuit templates", () => {
    ...
    expect(
      screen.getAllByText("10분 · 매우 조용한 리셋").length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText("15분 · 무도구 전신 협응").length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText("20분 · 저소음 전신 기본").length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText("30분 · 저소음 전신 리듬").length
    ).toBeGreaterThan(0);
    expect(screen.getAllByText(/소음 수준: 매우 낮음/).length).toBeGreaterThan(
      0
    );
    expect(
      screen.getAllByText(/점프·발 구르기 없이 작은 체중 이동/).length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText(/점프·발 구르기·가구 충격 없음/).length
    ).toBeGreaterThan(0);
  });
  ...
  it("renders life-stage general guidance and the pre-exercise safety checklist", () => {
    render(createElement(Home));
    expect(
      screen.getAllByText("운동 시작 전 5가지 확인").length
    ).toBeGreaterThan(0);
  ...
  it("renders nutrition, hydration, and heat-exposure safety guidance after expanding wellness cards", () => {
    ...
    ["운동 전후 식사", "사우나와 열 노출", "수분과 더운 날 활동"].forEach(
      title => {
        ...
        expect(card).toBeTruthy();
        fireEvent.click(
          within(card!).getByRole("button", { name: "상세 가이드" })
        );
      }
    );
    expect(screen.getAllByText(/의료·영양 전문가 상담/).length).toBeGreaterThan(
      0
    );
    expect(screen.getAllByText(/실신감·혼란·심한 두통/).length).toBeGreaterThan(
      0
    );
    expect(screen.getAllByText(/심한 두통·실신감·혼란/).length).toBeGreaterThan(
      0
    );
  });
  ...
  it("saves local category, equipment, and environment preferences then applies them to explore and session design", () => {
    ...
    fireEvent.change(screen.getByLabelText("선호 장비"), {
      target: { value: "bodyweight" },
    });
    fireEvent.change(screen.getByLabelText("주 활동 환경"), {
      target: { value: "outdoor" },
    });
    fireEvent.click(screen.getByRole("button", { name: "설정 저장" }));

    expect(
      JSON.parse(window.localStorage.getItem("fit-atlas-profile") ?? "{}")
    ).toMatchObject({
    ...
    expect(
      screen.getByRole("heading", { name: "30분 전신 균형 세션 · 야외·걷기" })
    ).toBeTruthy();

    const typeFilter = screen.getByRole("group", {
      name: "운동 종류 빠른 필터",
    });
    fireEvent.click(
      within(typeFilter).getByRole("button", { name: "선호 조건 적용" })
    );
    expect(
      within(typeFilter)
        .getByRole("button", { name: "맨몸운동" })
    ...
    expect(
      (screen.getByLabelText("장비 필터") as HTMLSelectElement).value
    ).toBe("장비 없음");
  });
  ...
  it("provides an accessible quick exercise-type filter that combines with existing filters and can reset", () => {
    ...
    const bodyweight = within(typeFilter).getByRole("button", {
      name: "맨몸운동",
    });
    ...
    fireEvent.change(screen.getByLabelText("장비 필터"), {
      target: { value: "장비 필요" },
    });
    expect(
      (screen.getByLabelText("장비 필터") as HTMLSelectElement).value
    ...
    fireEvent.click(
      within(typeFilter).getByRole("button", { name: "조건 초기화" })
    );
    expect(
      within(typeFilter)
  ...
  it("sorts filtered exercise results from the accessible sort dropdown and resets to recommendations", () => {
    ...
    fireEvent.click(
      within(typeFilter).getByRole("button", { name: "요가·필라테스" })
    );
    ...
    expect(
      within(typeFilter).getByText(/요가·필라테스 \d+개 표시/)
    ).toBeTruthy();
  ...
  it("keeps sorting visible and explains active conditions with a direct empty-result reset", () => {
    ...
    fireEvent.change(screen.getByLabelText("운동 검색"), {
      target: { value: "존재하지않는운동조건" },
    });
    expect(
      screen.getByRole("heading", { name: "일치하는 운동이 없습니다." })
    ).toBeTruthy();
    expect(screen.getByLabelText("적용된 탐색 조건").textContent).toContain(
      "검색 · 존재하지않는운동조건"
    );
    ...
    expect(
      (screen.getByLabelText("정렬 기준") as HTMLSelectElement).value
    ).toBe("recommended");
    expect(
      screen.queryByRole("heading", { name: "일치하는 운동이 없습니다." })
    ).toBeNull();
  });
  ...
  it("keeps mobile quick navigation and direct session and ROM starts available", async () => {
    ...
    expect(
      within(quickNav).getByRole("link", { name: "오늘 세션" })
    ).toBeTruthy();
    ...
    expect(screen.getByLabelText("세션 구성 비중").textContent).toMatch(
      /분 · \d+%/
    );
    const sessionSection = document.getElementById("session");
    expect(sessionSection).toBeTruthy();
    fireEvent.click(
      within(sessionSection!).getByRole("button", { name: "기초 근력" })
    );
    await waitFor(() =>
      expect(screen.getByLabelText("이전 세션 비중 비교")).toBeTruthy()
    );
    expect(screen.getByLabelText("세션 비중 변화량").textContent).toContain(
      "%p"
    );
    expect(
      screen.getByRole("button", { name: "이번 주 계획에 추가" })
    ).toBeTruthy();
  ...
  it("keeps exercise cards scannable before opening the detailed movement and evidence guidance", async () => {
    ...
    const selectedType = within(typeFilter)
      .getAllByRole("button")
  ...
  it("adds atlas coordinates to exercise cards and exposes wellness details as an expandable reading region", () => {
    ...
    const firstExerciseCard = screen
      .getAllByRole("button", { name: "자세·근거 보기" })[0]
      ?.closest("article");
    expect(firstExerciseCard?.getAttribute("data-atlas-index")).toBe("001");

    const wellnessCard = screen
      .getByRole("heading", { name: "운동 전후 식사" })
      .closest("article");
    expect(wellnessCard).toBeTruthy();
    const detailToggle = within(wellnessCard!).getByRole("button", {
      name: "상세 가이드",
    });
  ...
  it("keeps the mobile result location, selected-region comparison, and first-record outcome explicit", () => {
    render(createElement(Home));
    expect(
      screen.getByLabelText("모바일 탐색 결과 위치").textContent
    ).toContain("RESULT ROUTE");
    expect(
      screen.getByLabelText("선택 부위 운동 비교 요약").textContent
    ).toContain("공통 자극");
    const firstRecordRoute = screen.getByLabelText("첫 기록 다음 흐름");
    expect(
      within(firstRecordRoute).getByText(/운동·시간·RPE 입력/)
    ).toBeTruthy();
    expect(
      within(firstRecordRoute).getByText(/주간 흐름에 첫 신호 표시/)
    ).toBeTruthy();
  });
  ...
  it("keeps an empty progress dashboard focused on the weekly signal and first record action", () => {
    ...
    const volumeChart = screen
      .getByRole("heading", { name: "최근 7일 볼륨" })
      .closest("div");
  ...
  it("keeps the wellness route and short recovery start available in the reading flow", () => {
    ...
    const startRecovery = screen.getByRole("button", {
      name: "5분 가볍게 시작",
    });
    fireEvent.click(startRecovery);
    const durationSelector = screen.getByRole("group", {
      name: "앉은 자세 회복 루틴 시간 선택",
    });
    expect(
      within(durationSelector)
        .getByRole("button", { name: "5분" })
        .getAttribute("aria-pressed")
    ).toBe("true");
  });
  ...
  it("keeps recovery stages and the weekly report-to-session route explicit", () => {
    ...
    const nextSessionRoute =
      screen.getByLabelText("주간 리포트 다음 세션 경로");
    expect(
      within(nextSessionRoute).getByText("01 주간 신호 확인")
    ).toBeTruthy();
  ...
  it("feeds a changed weekly goal into the next session design", async () => {
    ...
    expect(
      screen.getByLabelText("주간 리포트 다음 세션 경로").textContent
    ).toContain("02 근력 세션 조절");
    ...
    expect(
      document.querySelector(".session-current-state")?.textContent
    ).toContain("기초 근력");
  });
  ...
  it("keeps the session plan separated into scannable coaching stages before save or log actions", () => {
    ...
    expect(
      Array.from(stages).map(
        stage => stage.querySelector(".small-label")?.textContent
      )
    ).toEqual(
      expect.arrayContaining([
        expect.stringContaining("준비"),
        expect.stringContaining("주요 움직임"),
        expect.stringContaining("마무리"),
      ])
    );
    expect(
      screen.getByRole("button", { name: "이번 주 계획에 추가" })
