...
export const recoveryPathways: RecoveryPathway[] = [
  ...
  { id: "wrist", label: "손목 불편", region: "팔", summary: "푸시업·플랭크·그립 운동이 불편할 때, 손바닥 체중 부하를 줄이고 전완·손목의 작은 범위 움직임으로 반응을 확인하는 경로입니다.", checkBefore: ["외상 뒤 변형·급성 붓기·손가락 감각 변화가 없는지 확인", "손을 쥘 수 없거나 통증이 빠르게 커지면 자가 진행보다 평가 우선"], chooseInstead: ["앉은 자세에서 손목을 작은 범위로 굽혔다 펴기", "손바닥 지지 대신 벽 슬라이드·하체·걷기 중심으로 전환"], returnRule: "일상적인 쥐기와 작은 손목 움직임이 다음 날까지 편안할 때, 지지 높이·시간·반복 중 하나만 조절하세요.", alternativeExerciseIds: ["seated-wrist-mobility", "wall-slide"], stopSignals: ["손가락 감각 저하 또는 뚜렷한 약화", "외상 뒤 변형·급성 붓기"] },
  ...
  { id: "hip", label: "고관절 불편", region: "둔근", summary: "깊은 스쿼트·런지·빠른 달리기가 불편할 때, 가동 범위를 강요하지 않고 보행·둔근 조절·작은 고관절 움직임으로 전환하는 경로입니다.", checkBefore: ["체중 부하 시 심한 통증·절뚝거림·갑작스러운 움직임 제한이 없는지 확인", "외상 뒤 변형·발열·밤에 깨는 지속 통증이면 자가 진행보다 평가 우선"], chooseInstead: ["반무릎 자세에서 고관절 앞쪽을 작은 범위로 편안하게 움직이기", "누운 자세의 낮은 브리지 또는 짧은 평지 걷기"], returnRule: "보행과 낮은 브리지를 다음 날까지 편안히 마친 뒤, 깊이·거리·저항 중 하나만 늘리세요.", alternativeExerciseIds: ["hip-flexor-stretch", "pilates-bridge"], stopSignals: ["체중 부하가 어려운 심한 통증", "외상 뒤 변형·발열 또는 진행성 절뚝거림"] },
];
