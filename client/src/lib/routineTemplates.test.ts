...
describe("routine templates", () => {
  it("covers all four primary goals with a safe four-week structure", () => {
    expect(routineTemplates.map((item) => item.goal)).toEqual(["strength", "endurance", "weight_management", "general_health"]);
    routineTemplates.forEach((template) => {
      ...
      template.weeks.forEach((week) => expect(week.focus.length).toBeGreaterThanOrEqual(3));
    });
  });
