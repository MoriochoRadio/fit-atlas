...
describe("explore preferences", () => {
  it("toggles favorites without duplicating identifiers", () => {
    const added = toggleFavoriteExercise({ favoriteExerciseIds: [], recentExerciseIds: [] }, "squat");
  ...
  it("keeps recently viewed exercises unique, most-recent first, and bounded", () => {
    const initial = { favoriteExerciseIds: [], recentExerciseIds: Array.from({ length: MAX_RECENT_EXERCISES }, (_, index) => `exercise-${index}`) };
  ...
  it("safely ignores malformed or duplicated stored identifiers", () => {
    expect(readExplorePreferences('{"favoriteExerciseIds":["squat","squat",4],"recentExerciseIds":["run","run",null]}')).toEqual({ favoriteExerciseIds: ["squat"], recentExerciseIds: ["run"] });
    expect(readExplorePreferences("not-json")).toEqual({ favoriteExerciseIds: [], recentExerciseIds: [] });
  });
});
