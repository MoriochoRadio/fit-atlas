...
type CatalogPageLoader = () => Promise<{
...
function getModuleEntries(
  ...
  if (!Array.isArray(entries))
    throw new Error(`카탈로그 페이지 ${pageIndex + 1}을 읽지 못했습니다.`);
...
export async function loadFullCatalog(): Promise<CatalogEntry[]> {
  const pages = await Promise.all(
    Array.from({ length: getCatalogPageCount() }, (_, pageIndex) =>
      loadCatalogPage(pageIndex)
    )
  );
  return pages.flat();
}

export async function loadCatalogEntriesByIds(
  exerciseIds: string[]
): Promise<CatalogEntry[]> {
  ...
  const pageIndexes = Array.from(
    new Set(
      requestedIds
        .map(id => catalogPageByExerciseId[id])
        .filter((pageIndex): pageIndex is number => Number.isInteger(pageIndex))
    )
  );
  const pages = await Promise.all(
    pageIndexes.map(pageIndex => loadCatalogPage(pageIndex))
  );
  const entriesById = new Map(
    pages.flat().map(entry => [entry.exercise.id, entry])
  );
...
export const entriesToExercises = (entries: CatalogEntry[]): Exercise[] =>
