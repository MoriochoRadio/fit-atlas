...
import {
  getAerobicIntensityInsight,
  getConsecutiveDayStreak,
  getConsistency,
  getExerciseTrend,
  getInsightSummary,
  getPersonalRecordTrend,
  getRegionBalance,
  getTrainingLoad,
...
describe("training insights", () => {
  ...
  it("aggregates recent session load and distinct active days", () => {
    expect(getTrainingLoad(logs, reference)).toEqual({
      load: 310,
      sessions: 2,
      minutes: 55,
    });
    expect(getConsistency(logs, reference)).toEqual({
      activeDays: 3,
      possibleDays: 28,
      weeklyAverage: 0.8,
    });
  ...
  it("maps exercise names to body-region balance and readable summaries", () => {
    expect(
      getRegionBalance(logs, catalog).some(
        item => item.region === "하체" && item.score > 0
      )
    ).toBe(true);
  ...
  it("reads aerobic RPE conservatively and compares a recent exercise trend", () => {
    ...
    expect(
      getAerobicIntensityInsight(expanded, reference, catalog)
    ).toMatchObject({
      sessions: 1,
      minutes: 30,
      distanceKm: 5,
    ...
    expect(getAerobicIntensityInsight(swim, reference, catalog).paceLabel).toBe(
      "평균 3분/100m"
    );
    const rowing = [
      {
    ...
    expect(
      getAerobicIntensityInsight(rowing, reference, catalog).paceLabel
    ).toBe("평균 2분/500m");
    expect(getExerciseTrend(expanded, reference)).toMatchObject({
      exercise: "바벨 백 스쿼트",
      direction: "상승",
    });
    expect(getConsecutiveDayStreak(expanded, reference)).toMatchObject({
      days: 3,
    });
    expect(getPersonalRecordTrend(expanded, reference)).toMatchObject({
      exercise: "바벨 백 스쿼트",
      direction: "새 PR",
    });
  });
