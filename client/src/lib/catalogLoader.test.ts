...
import { catalogSummary, entriesToExercises, getCatalogPageCount, getInitialCatalogEntries, loadCatalogEntriesByIds, loadCatalogPage, loadFullCatalog } from "./catalogLoader";
...
describe("lazy exercise catalog loader", () => {
  ...
  it("loads the compact eleventh page with complete detail pairs", async () => {
    ...
    expect(finalPage.every(({ detail }) => detail.commonMistakes.length >= 3 && detail.regressions.length >= 3)).toBe(true);
  });

  it("resolves non-contact racket and combat starter drills by id from their lazy catalog pages", async () => {
    const additions = await loadCatalogEntriesByIds(["tennis-ready-split-step-easy", "boxing-guard-step-reset-easy"]);
    expect(additions.map(({ exercise }) => exercise.id)).toEqual(["tennis-ready-split-step-easy", "boxing-guard-step-reset-easy"]);
    expect(additions.every(({ detail }) => detail.setup.length === 3 && detail.progressions.length === 3)).toBe(true);
  });
