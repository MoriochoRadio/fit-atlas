...
describe("low-noise home circuits", () => {
  it("provides distinct 10, 15, 20, and 30 minute no-jump templates", () => {
    expect(lowNoiseCircuitTemplates.map(template => template.id)).toEqual([
      "quiet-10",
      "quiet-15",
      "quiet-20",
      "quiet-30",
    ]);
    expect(lowNoiseCircuitTemplates.map(template => template.title)).toEqual(
  ...
  it("uses a quiet, wrist-friendly fallback in the 15 minute no-equipment coordination circuit", () => {
    const template = lowNoiseCircuitTemplates.find(
      item => item.id === "quiet-15"
    );
