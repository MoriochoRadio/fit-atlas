...
describe("recovery pathways", () => {
  it("provides seven education-only joint and movement pathways with safe alternatives", () => {
    ...
    recoveryPathways.forEach(pathway => {
      ...
      pathway.alternativeExerciseIds.forEach(id =>
        expect(exercises.some(exercise => exercise.id === id)).toBe(true)
      );
    });
  });
  ...
  it("returns the selected pathway for the body-map recovery flow", () => {
    ...
    expect(getRecoveryPathway("low-back").alternativeExerciseIds).toContain(
      "bird-dog"
    );
    expect(getRecoveryPathway("hip").alternativeExerciseIds).toContain(
      "wall-supported-lateral-reach"
    );
    expect(getRecoveryPathway("knee").alternativeExerciseIds).toContain(
      "seated-march-to-stand"
    );
    expect(getRecoveryPathway("wrist").alternativeExerciseIds).toContain(
      "seated-march-to-stand"
    );
  });

  it("creates the Home search and region action for an alternative exercise", () => {
    const action = getRecoveryExploreAction(getRecoveryPathway("ankle"), {
      name: "발목 니투월 락",
    });
  ...
  it("applies the Home search, region filter, and explore scroll effect together", () => {
    ...
    const result = applyRecoveryExplore(
      ...
      exercises,
      {
        ...
        scrollToTarget: value => calls.push(`scroll:${value}`),
      }
    );
  ...
  it("keeps new functional alternatives connected to a valid Home exploration action", () => {
    const result = applyRecoveryExplore(
      getRecoveryPathway("hip"),
      "wall-supported-lateral-reach",
      exercises,
      { applyFilters: () => undefined, scrollToTarget: () => undefined }
    );
