...
export type RecoveryRoutineRecord = {
  duration: SeatedRecoveryDuration;
  completedOn: string;
  completedAt: string | null;
  reflection: RecoveryReflection | null;
  note: string;
};
export type WellnessPreferences = {
  savedRecoveryDuration: SeatedRecoveryDuration | null;
  lastRecoveryRecord: RecoveryRoutineRecord | null;
  recoveryHistory: RecoveryRoutineRecord[];
};

export const defaultWellnessPreferences: WellnessPreferences = {
  savedRecoveryDuration: null,
...
function readRecoveryRecord(value: unknown): RecoveryRoutineRecord | null {
  ...
  if (
    !recoveryDurations.includes(record.duration as SeatedRecoveryDuration) ||
    typeof record.completedOn !== "string" ||
    record.completedOn.length === 0
  )
    return null;
  if (
  ...
  if (
    ...
    !recoveryReflections.includes(record.reflection as RecoveryReflection)
  )
    return null;
  return {
    duration: record.duration as SeatedRecoveryDuration,
    completedOn: record.completedOn,
    completedAt: record.completedAt ?? null,
    reflection: record.reflection as RecoveryReflection | null,
    note:
      typeof record.note === "string"
        ? record.note.slice(0, recoveryNoteMaxLength)
        : "",
  };
}
...
function readRecoveryHistory(value: unknown) {
  ...
  return value
    .map(readRecoveryRecord)
    .filter((record): record is RecoveryRoutineRecord => record !== null)
    .sort(
      (left, right) =>
        recoveryRecordTimestamp(right) - recoveryRecordTimestamp(left)
    )
    .slice(0, recoveryHistoryLimit);
}

export function readWellnessPreferences(
  ...
  try {
    ...
    return {
      savedRecoveryDuration: recoveryDurations.includes(
        value?.savedRecoveryDuration
      )
        ? value.savedRecoveryDuration
        : null,
      lastRecoveryRecord,
...
export function readLocalWellnessPreferences() {
  ...
  return readWellnessPreferences(
    window.localStorage.getItem(WELLNESS_PREFERENCES_KEY)
  );
}
