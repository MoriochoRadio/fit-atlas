import type { RecoveryReflection, RecoveryRoutineRecord } from "./wellnessPreferences";
...
export function summarizeRecoveryHistory(history: RecoveryRoutineRecord[]): RecoveryHistorySummary {
  const reflections: Record<RecoveryReflection, number> = { lighter: 0, same: 0, pause: 0 };
  ...
  const mostUsedDuration = history.length === 0 ? null : durations[10] > durations[5] ? 10 : 5;
