...
const verifiedSourcedExercises = exercises.filter(exercise =>
...
describe("exercise catalog quality gate", () => {
  it("keeps a catalog of individually named exercises instead of generated coaching combinations", () => {
    ...
    expect(
      exercises.filter(exercise => exercise.id.startsWith("verified-")).length
    ).toBe(534);
  });
  ...
  it("keeps new racket and combat starter drills non-contact with individualized safety and detail knowledge", () => {
    const ids = [
      ...
      "tennis-shadow-forehand-recovery-easy",
      "badminton-ready-side-step-easy",
      "badminton-net-step-return-easy",
      "table-tennis-ready-shift-easy",
      "boxing-guard-step-reset-easy",
      "boxing-shadow-jab-return-easy",
      "martial-arts-stance-shift-easy",
    ];
    ...
    additions.forEach(exercise => {
      ...
      expect(exercise.warning).toMatch(
        /통증|어지러움|미끄러움|장애물|대련|타격/
      );
  ...
  it("keeps the new expansion grounded in sourced independent movements, not set prescriptions", () => {
    ...
    verifiedSourcedExercises.forEach(exercise => {
      expect(exercise.reference.url).toMatch(
        /acsm\.org|github\.com\/yuhonas\/free-exercise-db/
      );
      expect(exercise.name).not.toMatch(
        /템포|폼 리셋|파셜 레인지|1\.5레프|포즈/
      );
      expect(exercise.englishName).not.toMatch(
        /tempo|form reset|partial range|one half rep|pause rep/i
      );
      ...
      expect(exercise.englishName).not.toMatch(
        /\s(easy|medium|hard|light|heavy)$/i
      );
    });
  });
  ...
  it("gives every sourced movement an exercise-specific instruction profile instead of a category template", () => {
    const instructionSignatures = verifiedSourcedExercises.map(exercise => {
      ...
      expect(exercise.description).not.toContain(
        "독립적인 동작 경로를 연습하는 실제 운동 종목입니다."
      );
      expect(exercise.cues.some(cue => cue.includes(exercise.name))).toBe(true);
      expect(
        exercise.benefits.some(benefit => benefit.includes(exercise.name))
      ).toBe(true);
      expect(detail.setup.some(step => step.includes(exercise.name))).toBe(
        true
      );
      expect(detail.finish).toContain(exercise.name);
      expect(
        detail.commonMistakes.every(mistake => mistake.includes(exercise.name))
      ).toBe(true);
      expect(
        detail.regressions.every(regression =>
          regression.includes(exercise.name)
        )
      ).toBe(true);
      expect(
        detail.progressions.every(progression =>
          progression.includes(exercise.name)
        )
      ).toBe(true);
      return normalize(
        JSON.stringify({
          ...
          setup: detail.setup.map(step =>
            step
              .replaceAll(exercise.name, "{name}")
              .replaceAll(exercise.equipment, "{equipment}")
          ),
          mistakes: detail.commonMistakes.map(mistake =>
            mistake.replaceAll(exercise.name, "{name}")
          ),
          regressions: detail.regressions.map(regression =>
            regression.replaceAll(exercise.name, "{name}")
          ),
          progressions: detail.progressions.map(progression =>
            progression.replaceAll(exercise.name, "{name}")
          ),
        })
      );
    });
    const counts = instructionSignatures.reduce<Record<string, number>>(
      (result, signature) => {
  ...
  it("excludes program structure and coaching methods from the independent exercise catalog", () => {
    expect(
      exercises.some(exercise => structuredTrainingEntryIds.has(exercise.id))
    ).toBe(false);
    expect(
      exercises.some(exercise =>
        /(?:behind(?: the)? neck|neck bridge|neck harness|judo flip|atlas stone|car deadlift|partner|with chains|with bands)/i.test(
  ...
  it("keeps identifiers and both display-name fields unique after normalization", () => {
    ...
    const englishNames = exercises.map(exercise =>
      normalize(exercise.englishName)
    );
