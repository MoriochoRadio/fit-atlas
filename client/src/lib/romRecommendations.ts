...
export function getRomRecommendation(presentation: AsciiDiagramPresentation): RomRecommendation {
  if (presentation.rom === "작음") return {
    ...
    alternatives: [{ name: "데드버그", rationale: "바닥 지지로 몸통을 안정시키며 작은 범위부터 연습" }, { name: "빠른 걷기", rationale: "통증 없는 리듬과 가벼운 체중 이동을 먼저 확인" }],
  ...
  if (presentation.rom === "큼") return {
    ...
    alternatives: [{ name: "리버스 런지", rationale: "보폭과 깊이를 스스로 줄여 하체 경로를 조절" }, { name: "바벨 백 스쿼트", rationale: "빈 바벨·박스 등으로 깊이와 부하를 단계적으로 조절" }],
  ...
  return {
    ...
    alternatives: [{ name: "스텝업", rationale: "스텝 높이로 움직임 경로를 보통 범위에서 조절" }, { name: "스테디 사이클", rationale: "저항을 낮춰 부드러운 반복 경로를 확인" }],
