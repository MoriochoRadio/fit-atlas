...
function readFilterPresets(value: unknown): ExploreFilterPreset[] {
  ...
  const presets = value.flatMap((item) => {
    ...
    const stringFields = [preset.id, preset.name, preset.keyword, preset.category, preset.focus, preset.region, preset.difficulty, preset.equipment, preset.sort, preset.rom];
    if (!stringFields.every((field) => typeof field === "string")) return [];
    const name = preset.name!.trim().slice(0, 28);
    if (!name || !preset.id) return [];
    return [{ id: preset.id.slice(0, 64), name, keyword: preset.keyword!.slice(0, 80), category: preset.category!.slice(0, 40), focus: preset.focus!.slice(0, 40), region: preset.region!.slice(0, 40), difficulty: preset.difficulty!.slice(0, 40), equipment: preset.equipment!.slice(0, 40), sort: preset.sort!.slice(0, 40), rom: preset.rom!.slice(0, 40) }];
  });
  return Array.from(new Map(presets.map((preset) => [preset.id, preset])).values()).slice(0, MAX_EXPLORE_FILTER_PRESETS);
}
...
export function saveExploreFilterPreset(preferences: ExplorePreferences, preset: ExploreFilterPreset): ExplorePreferences {
  return { ...preferences, filterPresets: [preset, ...preferences.filterPresets.filter((item) => item.id !== preset.id)].slice(0, MAX_EXPLORE_FILTER_PRESETS) };
}

export function removeExploreFilterPreset(preferences: ExplorePreferences, presetId: string): ExplorePreferences {
  return { ...preferences, filterPresets: preferences.filterPresets.filter((preset) => preset.id !== presetId) };
}
