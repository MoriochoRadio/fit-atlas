...
describe("single session builder", () => {
  it("builds a bounded session with the selected duration and environment", () => {
    ...
    expect(plan.blocks.reduce((total, block) => total + block.minutes, 0)).toBe(
      30
    );
  ...
  it("applies a lighter template when readiness is reduced", () => {
    const plan = buildSession({
      ...
      environment: "home",
      duration: 15,
      checkin: { ...checkin, energy: 2, sleep: 2, stress: 4, pain: 2 },
    });
    expect(plan.summary).toContain("낮추고");
    expect(plan.blocks[1].items.every(item => item.includes("가볍게"))).toBe(
      true
    );
  });
  ...
  it("surfaces expanded equipment, low-impact cardio, and coordination options in appropriate environments", () => {
    const home = buildSession({
      goal: "strength",
      environment: "home",
      duration: 45,
      checkin,
    });
    const gym = buildSession({
      goal: "all_round",
      environment: "gym",
      duration: 45,
      checkin,
    });
    const outdoor = buildSession({
      goal: "all_round",
      environment: "outdoor",
      duration: 45,
      checkin,
    ...
    expect(gym.blocks[1].items.join(" ")).toContain(
      "케이블 싱글 암 체스트 프레스"
    );
  ...
  it("includes the current squat, push, incline-walk, and machine progression variants", () => {
    ...
    const gymStrength = buildSession({
      goal: "strength",
      environment: "gym",
      duration: 45,
      checkin,
    });
    const gymEndurance = buildSession({
      goal: "endurance",
      environment: "gym",
      duration: 30,
      checkin,
    });
    expect(home.blocks[1].items.join(" ")).toContain(
      "지지 피스톨 스쿼트 투 박스"
    );
    expect(home.blocks[1].items.join(" ")).toContain("지지 풀업 네거티브");
    expect(gymStrength.blocks[1].items.join(" ")).toContain(
      "어시스트 뉴트럴 그립 풀업 머신"
    );
    expect(gymStrength.blocks[1].items.join(" ")).toContain("머신 앱 크런치");
    expect(gymEndurance.blocks[1].items.join(" ")).toContain(
      "트레드밀 조그·워크 인터벌"
    );
  });
  ...
  it("connects supported pull-up progressions and advanced balance variants", () => {
    const homeStrength = buildSession({
      goal: "strength",
      environment: "home",
      duration: 45,
      checkin,
    });
    const homeAllRound = buildSession({
      goal: "all_round",
      environment: "home",
      duration: 45,
      checkin,
    ...
    expect(homeStrength.blocks[1].items.join(" ")).toContain(
      "지지 풀업 네거티브"
    );
    expect(homeStrength.blocks[1].items.join(" ")).toContain(
      "발 지지 풀업 포즈 래더"
    );
    expect(homeAllRound.blocks[1].items.join(" ")).toContain(
      "지지 Y 밸런스 리치"
    );
    expect(homeAllRound.blocks[1].items.join(" ")).toContain(
      "지지 레터럴 런지"
  ...
  it("connects advanced pull-up transition and grip variation options", () => {
    ...
    const gymAllRound = buildSession({
      goal: "all_round",
      environment: "gym",
      duration: 45,
      checkin,
    ...
    expect(gymAllRound.blocks[1].items.join(" ")).toContain(
      "어시스트 뉴트럴 그립 풀업 머신"
    );
  });
  ...
  it("connects low-noise no-jump whole-body circuit options at home", () => {
    const homeEndurance = buildSession({
      goal: "endurance",
      environment: "home",
      duration: 30,
      checkin,
    ...
    expect(homeEndurance.blocks[1].items.join(" ")).toContain(
      "콰이어트 스텝 터치"
    );
    expect(homeEndurance.blocks[1].items.join(" ")).toContain(
      "스탠딩 섀도 복싱 이지"
  ...
  it("connects solo progression, split, hill, cadence, and ladder running options outdoors or on gym equipment", () => {
    ...
    const outdoorEndurance = buildSession({
      goal: "endurance",
      environment: "outdoor",
      duration: 45,
      checkin,
    });
    const outdoorAllRound = buildSession({
      goal: "all_round",
      environment: "outdoor",
      duration: 45,
      checkin,
    ...
    expect(outdoorEndurance.blocks[1].items.join(" ")).toContain(
      "이지 프로그레션 런"
    );
    expect(outdoorEndurance.blocks[1].items.join(" ")).toContain(
      "크루즈 인터벌 런 이지"
    ...
    expect(outdoorAllRound.blocks[1].items.join(" ")).toContain(
      "힐 리피트 워크·런 이지"
    );
    expect(outdoorAllRound.blocks[1].items.join(" ")).toContain(
      "케이던스 래더 마치·조그"
