...
export const preferredEquipmentOptions = [
  "flexible",
  "bodyweight",
  "basic_home",
  "gym",
...
export function readProfilePreferences(
  ...
  try {
    const parsed = serialized
      ? (JSON.parse(serialized) as Partial<ProfilePreferences>)
      : {};
    const candidate = { ...defaultProfilePreferences, ...parsed };
    return {
      ...candidate,
      preferredCategory: preferredCategoryOptions.includes(
        candidate.preferredCategory as PreferredCategory
      )
        ? (candidate.preferredCategory as PreferredCategory)
        : defaultProfilePreferences.preferredCategory,
      preferredEquipment: preferredEquipmentOptions.includes(
        candidate.preferredEquipment as PreferredEquipment
      )
        ? (candidate.preferredEquipment as PreferredEquipment)
        : defaultProfilePreferences.preferredEquipment,
      preferredEnvironment: preferredEnvironmentOptions.includes(
        candidate.preferredEnvironment as PreferredEnvironment
      )
        ? (candidate.preferredEnvironment as PreferredEnvironment)
        : defaultProfilePreferences.preferredEnvironment,
    };
  } catch {
