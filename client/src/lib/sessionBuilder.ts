...
const mainItems: Record<SessionGoal, Record<SessionEnvironment, string[]>> = {
  strength: {
    home: ["케틀벨 데드리프트 또는 지지 리버스 런지 · 2세트", "저항 밴드 로우 또는 카운터 인클라인 푸시업 · 2세트", "의자 일어서기·도달 또는 얕은 월 싯 · 2세트", "버드 독 · 좌우 6회"],
  ...
  endurance: {
    ...
    outdoor: ["이지 인클라인 워크 또는 트레킹 폴 워크 프렙", "야외 페이스 리셋 워크는 호흡이 편안할 때만", "더운 날에는 그늘·물·낮은 페이스 우선"],
  },
  all_round: {
    home: ["케틀벨 데드리프트 또는 의자 일어서기·도달 · 2세트", "저항 밴드 로우 또는 카운터 인클라인 푸시업 · 2세트", "라이트 수트케이스 캐리 또는 버드 독 · 좌우 2세트", "편안한 제자리 걷기"],
