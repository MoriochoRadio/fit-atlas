...
const mainItems: Record<SessionGoal, Record<SessionEnvironment, string[]>> = {
  strength: {
    home: ["케틀벨 데드리프트 또는 맨몸 굿모닝 또는 콰이어트 힙 힌지 리치 · 2세트", "지지 풀업 네거티브 또는 발 지지 오버핸드 풀업 또는 내로우 인클라인 푸시업 이지 · 2세트", "지지 피스톨 스쿼트 투 박스 또는 지지 레터럴 런지 또는 지지 스플릿 스쿼트 홀드 또는 솔레우스 월 레이즈 이지 · 2세트", "발 지지 풀업 포즈 래더 또는 데드 버그 콘트랄래터럴 리치 또는 니 플랭크 업다운 또는 데드 버그 아이소메트릭 프레스 또는 베어 플랭크 스캐풀라 프로트랙션 또는 포어암 플랭크 니 홀드 또는 수파인 90·90 힐 프레스 · 좌우 6회"],
  ...
  all_round: {
    home: ["스쿼트 투 카프 레이즈 또는 월 싯 카프 레이즈 이지 또는 콰이어트 스쿼트 프런트 리치 또는 티비알리스 월 레이즈 이지 또는 시티드 펠빅 틸트 이지 · 2세트", "로우 바 그립 전환 로우 또는 아처 푸시업 프렙 또는 셀프 레지스티드 로우 이지 또는 카운터 맨몸 트라이셉스 익스텐션 또는 월 푸시업 마치 이지 또는 월 슬라이드 리프트오프 이지 또는 월 스노우 엔젤 이지 · 2세트", "지지 Y 밸런스 리치 또는 지지 레터럴 런지 또는 지지 컨트롤 스텝백 탭 또는 지지 하프 니링 투 스탠드 또는 지지 스탠딩 클락 리치 · 좌우 2세트", "하이 플랭크 니 투 엘보 이지 또는 베어 크롤 포워드 이지 또는 포어암 플랭크 니 홀드 또는 수파인 90·90 힐 프레스 또는 프론 코브라 홀드 이지"],
