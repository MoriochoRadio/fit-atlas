...
const mainItems: Record<SessionGoal, Record<SessionEnvironment, string[]>> = {
  strength: {
    home: ["케틀벨 데드리프트 또는 높은 지지 힙 쓰러스트 · 2세트", "지지 인버티드 로우 또는 핸드 릴리스 푸시업 · 2세트", "지지 피스톨 스쿼트 투 박스 또는 지지 스케이터 스쿼트 · 2세트", "노르딕 컬 프렙 또는 버드 독 · 좌우 6회"],
    gym: ["레그 프레스 또는 샌드백 프런트 스쿼트 · 2–3세트", "머신 인클라인 체스트 프레스 또는 랜드마인 프레스 · 2–3세트", "체스트 서포티드 머신 로우 또는 케이블 스트레이트 암 풀다운 · 2–3세트", "머신 앱 크런치 또는 케이블 스탠딩 힙 어브덕션 · 2세트"],
    outdoor: ["난간 지지 스텝업 · 좌우 2세트", "난간 지지 스텝다운 · 좌우 2세트", "로우 스텝 레터럴 탭 · 좌우 2세트", "탠덤 스탠스 · 좌우 20초"],
  },
  endurance: {
    home: ["제자리 걷기 또는 로우 스텝 마치", "편안한 속도 2–4분 + 천천히 1분 반복", "가동성 1–2개를 회복 구간에 배치"],
    gym: ["리컴번트 바이크·일립티컬·트레드밀 인클라인 템포 워크 중 선택", "대화 가능한 강도에서 지속", "자세가 무너지면 저항을 낮추기"],
    outdoor: ["템포 워크·런 또는 러닝 액셀러레이션 마치", "경사 조그·워크는 호흡이 편안할 때만", "더운 날에는 그늘·물·낮은 페이스 우선"],
  },
  all_round: {
    home: ["케틀벨 데드리프트 또는 의자 일어서기·도달 · 2세트", "지지 인버티드 로우 또는 카운터 인클라인 푸시업 · 2세트", "리버스 테이블탑 홀드 또는 버드 독 · 좌우 2세트", "편안한 제자리 걷기"],
    gym: ["레그 프레스 또는 샌드백 프런트 스쿼트 · 2세트", "체스트 서포티드 로우 또는 어시스트 딥 머신 · 2세트", "일립티컬 이지 또는 트레드밀 템포 워크 · 편안한 강도", "케이블 스탠딩 힙 어브덕션 또는 팔로프 프레스 · 좌우 2세트"],
