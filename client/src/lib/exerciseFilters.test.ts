...
describe("exercise library QA", () => {
  it("covers every supported category with posture, benefits, warnings, and sources", () => {
    ...
    expect(exercises.map((exercise) => exercise.id)).toEqual(expect.arrayContaining(["kettlebell-sumo-deadlift", "resistance-band-row", "battle-rope-alternating-wave", "low-step-march", "medicine-ball-scoop-toss", "aqua-walk", "elliptical-easy", "recumbent-bike-easy", "row-erg-easy", "sandbag-bear-hug-carry", "trx-row", "landmine-press", "landmine-rotation-prep", "seated-wrist-mobility", "pool-easy-swim", "rail-supported-step-up", "counter-incline-pushup", "supported-floor-transfer", "grocery-bag-lift-to-counter", "easy-incline-walk", "trekking-pole-walk-prep", "light-daypack-walk", "outdoor-pace-reset", "cable-biceps-curl-easy", "machine-incline-chest-press-easy", "a-skip-walk-drill", "easy-fartlek-run", "pike-pushup-prep", "copenhagen-plank-knee-easy"]));
  });
