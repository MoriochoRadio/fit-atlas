...
describe("exercise library QA", () => {
  ...
  it("includes solo treadmill and walking movements without treating pace prescriptions as new exercises", () => {
    expect(exercises.map((exercise) => exercise.id)).toEqual(expect.arrayContaining(["incline-treadmill-walk", "treadmill-recovery-walk", "curved-treadmill-walk-easy", "treadmill-cooldown-walk"]));
    expect(filterExercises(exercises, { ...allFilters, keyword: "트레드밀" }).map((exercise) => exercise.id)).toContain("incline-treadmill-walk");
  });
  ...
  it("includes the 500-exercise expansion across running, gym equipment, bodyweight, balance, and mobility", () => {
    expect(exercises.map((exercise) => exercise.id)).toEqual(expect.arrayContaining(["incline-treadmill-walk", "treadmill-recovery-walk", "cable-kneeling-chop-easy", "machine-hip-abduction-isometric-easy", "wall-pike-shoulder-tap-easy", "bear-crawl-lateral-easy", "clock-balance-reach-easy", "supine-windshield-wiper-easy"]));
    expect(filterExercises(exercises, { ...allFilters, category: "맨몸운동", keyword: "베어 크롤" }).map((exercise) => exercise.id)).toContain("bear-crawl-lateral-easy");
  });
